...
@section('title', 'Práctica: ' . ($desafio->titulo ?? 'Desafío'))
@section('content')
<!-- ...existing code (encabezado, breadcrumb, etc.) ... -->

<div class="container py-4">
	<a href="{{ route('student.practica.index') }}" class="btn btn-link mb-3">&larr; Volver a Prácticas</a>
	...
	<div class="practice-list">
		<div class="practice-card">
			<div class="practice-header">
				...
				<div>
					<h3 class="practice-title">{{ $desafio->titulo ?? 'Desafío' }}</h3>
					<div class="practice-meta">
						@if($curso) Curso: <strong>{{ $curso->titulo }}</strong> · @endif
						Nivel: <strong>{{ $desafio->nivel ?? ($curso->nivel ?? 'N/A') }}</strong>
					</div>
				</div>
			</div>

			<p class="practice-desc mt-3">{{ $desafio->descripcion ?? 'Descripción no disponible.' }}</p>

			<div class="practice-footer">
				<div class="practice-badges">
					<span class="badge-small">{{ $desafio->puntos ?? '0' }} puntos</span>
					<span class="badge-small">{{ $desafio->tipo ?? 'Ejercicio' }}</span>
					@if($curso)
						<span class="badge-small">{{ $curso->lenguaje->nombre ?? 'Lenguaje' }}</span>
					@endif
				</div>

				<button type="button" class="solve-btn" data-bs-toggle="modal" data-bs-target="#solveModal" data-challenge-id="{{ $desafio->challenge_id ?? $desafio->id }}">
		...
		<div class="mt-4">
			...
			@if($lecciones && $lecciones->count())
				<ul class="list-group">
					@foreach($lecciones as $leccion)
						<li class="list-group-item d-flex justify-content-between align-items-center">
							<div>
								<strong>{{ $leccion->titulo ?? 'Lección' }}</strong>
								<div class="small text-muted">{{ Str::limit($leccion->descripcion ?? '', 120) }}</div>
							</div>
							<span class="badge bg-secondary">{{ $leccion->duracion ?? '—' }}</span>
						</li>
					@endforeach
				</ul>
			@else
  ...
  <div class="modal-dialog modal-xl modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="solveModalLabel">Resolver: {{ $desafio->titulo ?? 'Desafío' }}</h5>
        ...
        <div class="editor-split">
            ...
            <div>
                <div class="lesson-summary mb-3">
                    ...
                    <div class="meta">
                        @if($curso) Curso: {{ $curso->titulo }} · @endif
                        Desafío: {{ $desafio->titulo ?? '—' }} · Puntos: {{ $desafio->puntos ?? 0 }}
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const TESTCASES = @json($desafio->testcases ?? []);
