...
<body>
	<nav class="navbar navbar-expand-lg navbar-dark">
		<div class="container">
			<a class="navbar-brand d-flex align-items-center" href="#">
				<img src="{{ asset('img/Logo.jpeg') }}" alt="SLYCIPHER" class="logo me-2" onerror="this.style.display='none'">
			...
			<div class="collapse navbar-collapse" id="nav">
				<div class="ms-auto d-flex align-items-center">
					@if (Route::has('login'))
						@guest
							<a id="btn-login-nav" href="{{ route('login') }}" class="btn btn-primary me-2">Iniciar Sesión</a>
							@if (Route::has('register'))
								<a id="btn-register-nav" href="{{ route('register') }}" class="btn btn-primary">Registrarse</a>
							@endif
						@else
							<!-- Usuario autenticado: los botones hacen logout y luego redirigen -->
							<button id="btn-login-nav" class="btn btn-primary me-2" type="button" onclick="logoutAndRedirect('{{ route('login') }}')">Iniciar Sesión</button>
							@if (Route::has('register'))
								<button id="btn-register-nav" class="btn btn-primary" type="button" onclick="logoutAndRedirect('{{ route('register') }}')">Registrarse</button>
							@endif
	...
	<section class="auth-container">
		<div class="container">
			<img src="{{ asset('img/Logo.jpeg') }}" alt="SLYCIPHER" class="logo">
			...
			@guest
				<div class="d-flex justify-content-center gap-2 mt-3">
					<a id="btn-login-hero" href="{{ route('login') }}" class="btn btn-primary">
						<i class="fas fa-sign-in-alt me-1"></i> Iniciar Sesión
					</a>
					@if (Route::has('register'))
						<a id="btn-register-hero" href="{{ route('register') }}" class="btn btn-primary">Registrarse</a>
					@endif
				</div>
			@endguest
		</div>
		...
		<div class="container">
			<div class="row align-items-center">
				<div class="col-lg-6">
					...
					@guest
						<div class="mt-4">
							<a id="btn-login-main" href="{{ route('login') }}" class="btn btn-primary me-2">
								<i class="fas fa-sign-in-alt me-1"></i> Iniciar Sesión
							</a>
							@if (Route::has('register'))
								<a id="btn-register-main" href="{{ route('register') }}" class="btn btn-primary">Registrarse</a>
							@endif
						</div>
					@else
						<div class="mt-4">
							<button id="btn-login-main" class="btn btn-primary me-2" type="button" onclick="logoutAndRedirect('{{ route('login') }}')">
								<i class="fas fa-sign-in-alt me-1"></i> Iniciar Sesión
							</button>
							@if (Route::has('register'))
								<button id="btn-register-main" class="btn btn-primary" type="button" onclick="logoutAndRedirect('{{ route('register') }}')">
									Registrarse
								</button>
							@endif
						</div>
	...
	@auth
		<div class="quick-actions" role="navigation" aria-label="Acciones rápidas">
			...
			{{-- Proponer desafío: visible para desarrollador --}}
			@if($role === 'desarrollador' || $role === 'developer')
				@if(Route::has('developer.desafios.create'))
					<a href="{{ route('developer.desafios.create') }}" class="qa-btn qa-create" title="Proponer nuevo desafío">
				...
				@if(Route::has('developer.lecciones.create'))
					<a href="{{ route('developer.lecciones.create') }}" class="qa-btn qa-lesson" title="Proponer nueva lección">
			...
			{{-- Revisiones: visible para admin --}}
			@if($role === 'admin' || $role === 'administrador')
				@if(Route::has('admin.reviews.index'))
					<a href="{{ route('admin.reviews.index') }}" class="qa-btn qa-review" title="Revisar propuestas">
	...
	<footer class="py-4 text-center text-muted" style="background:rgb(252, 248, 248)">
		<div class="container">© {{ date('Y') }} SLYCIPHER</div>
	</footer>
